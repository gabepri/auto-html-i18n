...
- **Translator** — Coordinates cache lookup → masking → resolution → DOM update. Tracks pending nodes by cache key (multiple nodes can share one key, resolved atomically). Remembers last-applied output per element/attribute (WeakMaps) to skip its own mutation echoes, re-translate framework-patched content, and skip stale pending applies.
